...
export default function ModuloComparacionPage() {
  ...
  const getChartData = (val1: number, val2: number) => {
    if (!hasData) return [];
    return [
      { label: confirmedLoc1.nombre, value: val1, subtitle: confirmedLoc1.estado ? `(${confirmedLoc1.estado})` : "" },
      { label: confirmedLoc2.nombre, value: val2, subtitle: confirmedLoc2.estado ? `(${confirmedLoc2.estado})` : "" },
    ];
  };

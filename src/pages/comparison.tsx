...
export default function ModuloComparacionPage() {
  ...
  const handleCompare = () => {
    ...
    console.log("Datos actualizados en gráficas");
  };

  const handleExportAction = (format: string) => {
    setTimeout(() => setIsExportOpen(false), 3000);
  };
  const getChartData = (val1: number, val2: number) => {
    ...
    return [
      { 
        label: confirmedLoc1.nombre || 'Ubicación 1', 
        value: val1, 
        subtitle: confirmedLoc1.estado ? `(${confirmedLoc1.estado})` : "" 
      },
      { 
        label: confirmedLoc2.nombre || 'Ubicación 2', 
        value: val2, 
      ...
      <div className="p-8 bg-[#F8FAFC] min-h-screen font-sans">
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="text-gray-500 font-semibold mt-1">
              Búsqueda de indicadores de salud | 2026
            </p>
          </div>
          <Button label="Exportar" buttonType="download" tone="blue" height="40" onClick={() => setIsExportOpen(true)} />
        </div>
        ...
        <div className="bg-white rounded-[40px] p-10 shadow-sm border border-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <PriorityCard 
              title={confirmedLoc1.nombre || "Selecciona ubicación"} 
              subtitle={confirmedLoc1.estado ? `(${confirmedLoc1.estado})` : ""}
              priority="alta" progress={85}
              metrics={[{ id: '1', label: "Cobertura Médica", value: "0.7" }]}
            />
            <PriorityCard 
              title={confirmedLoc2.nombre || "Selecciona ubicación"} 
              subtitle={confirmedLoc2.estado ? `(${confirmedLoc2.estado})` : ""}
              priority="baja" progress={25}
              metrics={[{ id: '1', label: "Cobertura Médica", value: "2.9" }]}
            />
          </div>
        </div>
      </div>

      <Export isOpen={isExportOpen} onClose={() => setIsExportOpen(false)} onExport={handleExportAction} />
    </AppLayout>
  );
}